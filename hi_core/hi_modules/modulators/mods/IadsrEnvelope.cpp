...
Processor * IadsrEnvelope::getChildProcessor(int processorIndex)
{
	jassert(processorIndex < internalChains.size());
	return internalChains[processorIndex].getChain();
}

const Processor * IadsrEnvelope::getChildProcessor(int processorIndex) const
{
	jassert(processorIndex < internalChains.size());
	return internalChains[processorIndex].getChain();
}

IadsrEnvelope::IadsrEnvelope(MainController *mc, const String &id, int voiceAmount, Modulation::Mode m) :
	...
	Modulation(m)
{
	...
	displayBuffer->setPropertyObject(new IadsrEnvelope::AhdsrRingBufferProperties(this));
...
void IadsrEnvelope::restoreFromValueTree(const ValueTree &v)
{
...
ValueTree IadsrEnvelope::exportAsValueTree() const
{
...
float IadsrEnvelope::startVoice(int voiceIndex)
{
	stateInfo.state = IadsrEnvelopeState::ATTACK;
	stateInfo.changeTime = getMainController()->getUptime();

	if (isMonophonic)
	{
		state = static_cast<IadsrEnvelopeState*>(monophonicState.get());
		...
		if (restartEnvelope)
		{
			...
			if (shouldRetrigger && state->current_state != IadsrEnvelopeState::IDLE)
			{
				state->current_state = IadsrEnvelopeState::RETRIGGER;
			}
			else
			{
				state->current_state = IadsrEnvelopeState::ATTACK;
	...
	{
		state = static_cast<IadsrEnvelopeState*>(states[voiceIndex]);

		if (state->current_state != IadsrEnvelopeState::IDLE)
		{
			reset(voiceIndex);
		}
...
void IadsrEnvelope::stopVoice(int voiceIndex)
{
	if (isMonophonic)
	{
	...
	{
		static_cast<IadsrEnvelopeState*>(states[voiceIndex])->current_state = IadsrEnvelopeState::RELEASE;
	}
}

void IadsrEnvelope::calculateBlock(int startSample, int numSamples)
{
	...
	const bool isSustain = static_cast<IadsrEnvelopeState*>(state)->current_state == IadsrEnvelopeState::SUSTAIN;
...
void IadsrEnvelope::reset(int voiceIndex)
{
	if (isMonophonic)
	{
		stateInfo.state = IadsrEnvelopeState::IDLE;
		return;
	}
	else
	{
		...
		state = static_cast<IadsrEnvelopeState*>(states[voiceIndex]);
		state->current_state = IadsrEnvelopeState::IDLE;
		state->current_value = 0.0f;
	}
}

void IadsrEnvelope::handleHiseEvent(const HiseEvent &e)
{
	EnvelopeModulator::handleHiseEvent(e);

	if (isInMonophonicMode() && getNumPressedKeys() == 0)
	{
		auto monoState = static_cast<IadsrEnvelopeState*>(monophonicState.get());
		monoState->current_state = IadsrEnvelopeState::RELEASE;
	}
...
float IadsrEnvelope::getDefaultValue(int parameterIndex) const
{
...
void IadsrEnvelope::setInternalAttribute(int parameterIndex, float newValue)
{
...
float IadsrEnvelope::getAttribute(int parameterIndex) const
{
	if (parameterIndex < EnvelopeModulator::Parameters::numParameters)
	{
...
void IadsrEnvelope::prepareToPlay(double sampleRate, int samplesPerBlock)
{
...
bool IadsrEnvelope::isPlaying(int voiceIndex) const
{
	if (isMonophonic)
		return true;

	return static_cast<IadsrEnvelopeState*>(states[voiceIndex])->current_state != IadsrEnvelopeState::IDLE;
}

EnvelopeModulator::ModulatorState * IadsrEnvelope::createSubclassedState(int voiceIndex) const
{
	return new IadsrEnvelopeState(voiceIndex, this);
}

float IadsrEnvelope::calculateNewValue(int /*voiceIndex*/)
{
	return state->tick();
}


ProcessorEditorBody * IadsrEnvelope::createEditor(ProcessorEditor* parentEditor)
{
#if USE_BACKEND

	return new IadsrEnvelopeEditor(parentEditor);
...
IadsrEnvelope::Panel::Panel(FloatingTile* parent) :
...
juce::Component* IadsrEnvelope::Panel::createContentComponent(int /*index*/)
{
...
void IadsrEnvelope::Panel::fillModuleList(StringArray& moduleList)
{
	fillModuleListWithType<IadsrEnvelope>(moduleList);
}
