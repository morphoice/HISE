...
#ifndef HI_IADSRENVELOPE_H_INCLUDED
#define HI_IADSRENVELOPE_H_INCLUDED
...
/** @brief An  envelope type with intial Level.
### IADSR Envelope
...
The Modulator has five states: Attack, Decay, Sustain and Release and allows modulation of
...
*/
class IadsrEnvelope: public EnvelopeModulator,
...
public:
	...
	/// @brief special parameters for IadsrEnvelope
	...
	IadsrEnvelope(MainController *mc, const String &id, int voiceAmount, Modulation::Mode m);
	~IadsrEnvelope() {};
	...
	public:
		...
		Identifier getProcessorTypeId() const override { return IadsrEnvelope::getClassType(); }
...
private:
	...
	struct IadsrEnvelopeState : public EnvelopeModulator::ModulatorState,
								public state_base
	{
		IadsrEnvelopeState(int voiceIndex, const ahdsr_base *ownerBase) :
	...
	StateInfo stateInfo;
	IadsrEnvelopeState *state;

	ModulatorChain::Collection internalChains;

	JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(IadsrEnvelope)
	JUCE_DECLARE_WEAK_REFERENCEABLE(IadsrEnvelope);
};
