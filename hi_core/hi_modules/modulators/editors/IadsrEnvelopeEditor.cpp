...
//==============================================================================
IadsrEnvelopeEditor::IadsrEnvelopeEditor (ProcessorEditor *p)
    : ProcessorEditorBody(p)
{
	...
	attackSlider->setup(getProcessor(), IadsrEnvelope::Attack, "Attack Time");
	attackSlider->setMode(HiSlider::Time);

	attackLevelSlider->setup(getProcessor(), IadsrEnvelope::AttackLevel, "Attack Level");
	attackLevelSlider->setMode(HiSlider::Decibel, -100.0, 0.0, -12.0);

	sustainSlider->setup(getProcessor(), IadsrEnvelope::Sustain, "Sustain Level");
	sustainSlider->setMode(HiSlider::Decibel, -100.0, 0.0, -12.0);

	holdSlider->setup(getProcessor(), IadsrEnvelope::Hold, "Hold Time");
	holdSlider->setMode(HiSlider::Time);

	decaySlider->setup(getProcessor(), IadsrEnvelope::Decay, "Decay Time");
	decaySlider->setMode(HiSlider::Time);

	releaseSlider->setup(getProcessor(), IadsrEnvelope::Release, "Release Time");
	releaseSlider->setMode(HiSlider::Time);

    label->setFont (GLOBAL_BOLD_FONT().withHeight(26.0f));

	attackCurveSlider->setup(getProcessor(), IadsrEnvelope::AttackCurve, "Attack Curve");
	attackCurveSlider->setMode(HiSlider::NormalizedPercentage);

	decayCurveSlider->setup(getProcessor(), IadsrEnvelope::DecayCurve, "Decay Curve");
...
IadsrEnvelopeEditor::~IadsrEnvelopeEditor()
{
...
//==============================================================================
void IadsrEnvelopeEditor::paint (Graphics& g)
{
...
void IadsrEnvelopeEditor::resized()
{
...
void IadsrEnvelopeEditor::sliderValueChanged (Slider* sliderThatWasMoved)
{
...
<JUCER_COMPONENT documentType="Component" className="IadsrEnvelopeEditor" componentName=""
